...
using EryfitProxy.Kernel.Core.Pcap.Cli.Clients;
...
namespace EryfitProxy.Kernel.Cli.Commands
{
    public class StartCommandBuilder
    {
        ...
        private DirectoryInfo TempDumpDirectory {
            get
            {
                ...
                var path = Path.Combine(Environment.ExpandEnvironmentVariables("%TEMP%"),
                    "eryfit.cli", _instanceIdentifier);
        ...
        {
            var proxyStartUpSetting = EryfitSetting.CreateDefault();
            ...
            EryfitSharedSetting.Use528 = !use502;
            ...
            if (requestBuffer.HasValue && requestBuffer >= 16) {
                EryfitSharedSetting.RequestProcessingBuffer = requestBuffer.Value;
            }

            EryfitSharedSetting.MaxConnectionPerHost = maxConnectionPerHost;
            ...
            await using var scope = new ProxyScope(() => new EryfitNetOutOfProcessHost(), a => new OutOfProcessCaptureContext(a));
        ...
        private static bool ValidateSetting(InvocationContext invocationContext, EryfitSetting proxyStartUpSetting)
        {
            var validationResults = AggregateEryfitSettingAnalyzer.Instance.Validate(proxyStartUpSetting).ToList();
    ...
    {
        public static readonly string[] DefaultRuleFileNames = { "eryfit-rule.yaml", "eryfit-rule.yml" };
