namespace EryfitProxy.Kernel.Cli.Commands
{
    public class PackCommandBuilder
    {
        ...
        {
            var command = new Command("pack", "Export a eryfit result directory to a specific archive format");
        ...
        private static Argument<DirectoryInfo> BuildInputDirectoryOption()
        {
            var argument = new Argument<DirectoryInfo>(
                "input-directory",
                description: "a eryfit folder result to export",
        ...
        private static Option<string> BuildOutputFileFormatOption()
        {
            var option = new Option<string>(
                "f",
                description: "The output file format among eryfit, har and saz",
