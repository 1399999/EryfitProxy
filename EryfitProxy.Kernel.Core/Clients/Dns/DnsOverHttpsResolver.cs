...
namespace EryfitProxy.Kernel.Clients.Dns
{
    internal class DnsOverHttpsResolver : DefaultDnsResolver
    {
        private static readonly Dictionary<string, string> DnsOverHttpsDefaultUrl = new(StringComparer.OrdinalIgnoreCase) {
            ["GOOGLE"] = Environment.GetEnvironmentVariable("ERYFIT_DEFAULT_GOOGLE_DNS_URL") ?? "https://8.8.8.8/resolve",
            ["CLOUDFLARE"] = Environment.GetEnvironmentVariable("ERYFIT_DEFAULT_CLOUDFLARE_DNS_URL") ?? "https://1.1.1.1/dns-query",
        };
        ...
        private async Task<List<string?>> InternalGetDnsData(string type, string hostName)
        {
            ...
            {
                throw new EryfitException("Failed to resolve DNS over HTTPS");
            }
