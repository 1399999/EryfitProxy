...
{
    [ActionMetadata(
        "Forward request to a specific URL. This action makes eryfit act as a reverse proxy. " +
        "Unlike [SpoofDnsAction](https://www.eryfit.io/rule/item/spoofDnsAction), host header is automatically set and protocol switch is supported (http to https, http/1.1 to h2, ...). The URL must be an absolute path.")]
